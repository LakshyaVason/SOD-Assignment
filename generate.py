...
#             if row[employee] == 'Yes' or row[employee] == 'yes':
...
# Step 2: Clean and preprocess the data if necessary
df = df.map(lambda x: str(x).strip().lower())  # Ensure all text data is lowercase and stripped of spaces
...
def assign_shifts(df):
    ...
    for index, row in df.iterrows():
        for employee in employees:
            if row[employee] == 'yes':  # Check availability using lowercase comparison
                availability[row['Date']].append(employee)
    ...
    for date in dates:
        ...
        if available_employees:  # Check if there are available employees
            ...
            # If no one is available, reset last_two_weeks to allow previous employees to be reused
            if len(availability[date]) > 0:  # Check if there were any available employees initially
                shuffle(availability[date])  # Shuffle the list of initially available employees
                assigned_employee = availability[date][0]  # Reassign even if it breaks the two-week rule
                assignments.append((date, assigned_employee))
                employee_count[assigned_employee] += 1
                last_two_weeks.append(assigned_employee)
                if len(last_two_weeks) > 10:
                    last_two_weeks.pop(0)
            else:
                assignments.append((date, 'Noone available'))  # Handle no available employee case
...
for row in sheet.iter_rows(min_row=2, min_col=1, max_col=1):  # Assuming dates are in column 1 starting from row 2
    for cell in row:
        if cell.value:
            try:
                cell_date = datetime.strptime(str(cell.value), '%Y-%m-%d %H:%M:%S')
                if cell_date < datetime.now():
                    cell.fill = past_date_fill
            except ValueError:
                pass  # Handle the error if the date format is incorrect
